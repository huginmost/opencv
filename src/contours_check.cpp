...
int main()
{
	string path = "../resources/armor1.jpg";
	Mat img = imread(path);
  cv::resize(img, img, cv::Size(img.cols * 0.5, img.rows * 0.5));
	Mat imgGray, binary, imgBlur, imgCanny, imgDil;

	// Preprocessing
	cvtColor(img, imgGray, COLOR_BGR2GRAY);

  threshold(imgGray, binary, 160, 255, THRESH_BINARY);
  
	GaussianBlur(binary, imgBlur, Size(3, 3), 3, 0);
